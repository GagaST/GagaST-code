...
class XDAEDenoisingPairDataset(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            source, target = tokens[self.tag_num:], tokens.clone()
        ...
        return {
            "id": index,
            "source": torch.cat((tokens[:self.tag_num], source), dim=0),
            "target": target,
        }
    ...
    def collater(self, samples):
        ...
        """
        return collate(samples, self.vocab.pad(), self.vocab)
